...
export async function POST(request: NextRequest) {
  ...
  if (!userId || (session.user as typeof session.user & { invalid?: boolean }).invalid) {
  ...
  const response = await fetch(`${baseUrl.replace(/\/$/, '')}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      temperature: 0.2,
      messages: [
        { role: 'system', content: 'You are a professional-networking reviewer. Return concise risks, evidence gaps, next actions, uncertainty, and decisions requiring human approval.' },
  ...
  await prisma.runtimeAiResult.create({ data: { id, userId, prompt, content, provider: 'openrouter', model, providerReceipt } });
